...
def main():
    ...
    esc=html.escape
    md=['# AssignmentHub v1.3.0 시나리오별 사용 예시','',
    ...
    page=f'''<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AssignmentHub v1.3.0 시나리오별 사용자 매뉴얼</title><style>{css}</style></head><body><header><h1>AssignmentHub 사용자 매뉴얼</h1><p>v1.3.0 · 무설치 실행 · TSV 명단 입력 · 12개 사용 시나리오</p><p class="downloads"><a href="AssignmentHub_퀵가이드.pdf">2쪽 퀵가이드</a> · <a href="AssignmentHub_사용자_매뉴얼.pdf">상세 PDF</a> · <a href="AssignmentHub_사용자_매뉴얼.pptx">편집용 PPTX</a></p><p>예시 인물과 수업은 가상입니다. 캡처의 로컬 주소와 임시 경로는 실제 교육장 IP와 운영용 저장 폴더로 바꿔 사용하세요.</p></header><main><nav aria-label="시나리오 목차">{''.join(nav)}</nav><article>{''.join(sections)}</article></main><footer>2026-09-08 개정 · 모든 화면과 예시는 이 폴더에서 오프라인으로 볼 수 있습니다.</footer></body></html>'''

...
운영자 PC에서 `start.bat`를 더블클릭합니다. Python 3.12와 Tcl/Tk가 필요합니다. 처음에는 설치 로그가 보이고, 설치가 끝나면 한국어 관리창이 열립니다. 오류가 있으면 콘솔을 닫지 않고 원인과 재설치 안내를 표시합니다. 설치 이후 일상적인 서버 운영에 명령어를 입력할 필요가 없습니다.
...
**새 과정 만들기**에서 과정명, 안내 IP, 관리자 ID와 비밀번호를 입력합니다. 비밀번호는 12~128자이며 확인란과 같아야 합니다. 평문 비밀번호는 설정 파일에 저장하지 않습니다.
...
- 관리창에서 과정을 선택하고 **서버 시작**을 누릅니다. 시작·중지는 백그라운드 작업으로 실행하고 경과 시간을 표시합니다. 표시 막대는 완료율이 아닌 작업 중 표시입니다.
...
검증 결과와 미검증 범위는 [검증 보고서의 서버 사용성 개선 항목](test-report.md#서버-사용성-개선-후속-검증)을 확인하세요. Tk 위젯 콜백 호출과 실제 프로세스·브라우저 테스트는 OS 마우스 입력 자동화와 구분합니다.

...
## 디자인·화면 크기 대응 검증 (2026-09-08)

**탭 정렬 후속 수정:** 기본 테마의 선택 탭 전용 여백을 제거하고 선택 배경·글자색으로 구분했습니다. 실제 Tk 탭의 클릭 영역을 측정하여 740 × 760과 540 × 480 창에서 각각 기본 정보 → 용량·전송 설정 → 기본 정보로 전환하는 6개 경우의 상단·하단 좌표가 모두 동일함을 확인했습니다. 기존 작은 창·키보드 이동 검사도 통과했습니다. PPT 3–4쪽의 두 실제 캡처와 설명을 갱신했습니다. 업로드 전체 시나리오는 이 탭 스타일 수정에서는 다시 실행하지 않았습니다.

[27쪽 PPT와 실제 캡처 24개](manual/README.md)에 공통 디자인과 작은 화면 예시를 반영했습니다. 아래는 디자인 변경 후의 실행 결과이며 이전 검증 기록과 구분합니다.

- `.venv\Scripts\python.exe -m pytest -q`: 당시 전체 **67개 통과, 150.26초**. 기존 Starlette/AnyIO 사용 중단 경고 1건이 있으며 실패는 없습니다.
- 추가한 `tests/test_desktop_layout.py`: **1개 통과**. 640 × 520, 800 × 600, 1100 × 760으로 축소·확대할 때 버튼 접근성, 내용 높이 갱신, 540 × 480 설정 창의 키보드 이동, 440 × 320 진단창의 닫기 버튼을 확인했습니다.
- `scripts/capture_manual.py`: 실제 예시 과정 생성·시작·진단·중지, 템플릿 명단 등록, 임시비밀번호 CSV 다운로드, 학생 비밀번호 변경, 연결 코드 사용, 파일 선택·제출·이어 올리기·다운로드를 통과했습니다. [실행 결과](manual/walkthrough.json)
- 로그인, 긴 파일명이 있는 제출 화면, 관리자 제출 현황의 **360/390/600/768/1024/1440px** 폭에서 가로 넘침이 없었습니다. 총 **18개 화면·너비 조합**입니다. 파일 드래그 앤 드롭도 실제 브라우저 이벤트로 확인했습니다.
- **11,400,000바이트**를 제출하면서 **8,388,608바이트** 이후 전송을 끊고 재로그인해 이어 올렸습니다. 다운로드 SHA-256은 원본과 일치했고 브라우저 JavaScript 오류는 0건입니다.
- PPT는 편집 가능한 설명과 최신 실제 캡처로 구성했습니다. 패키지·글꼴·슬라이드 배치 검사와 Artifact Tool 재읽기를 통과했으며 PowerPoint에서 전체 슬라이드를 렌더링해 확인했습니다.

브라우저 뷰포트 검사는 실제 스마트폰·터치 입력 시험과 구분합니다. 다른 DPI·물리 PC의 LAN 접속·TLS 운영·프로젝터는 이번 디자인 변경의 검증 범위에 포함하지 않았습니다.
